...
function buildResultText(result, form) {
  return [
    ...
    `Chế phẩm I M O cần dùng ${result.imoMl} mi li lít.`,
    ...
    result.compostPerM2 ? `Mật độ phân bón: ${result.compostPerM2} ki lô gam trên mét vuông.` : "",
    `Lịch tưới: ${form.cropType} tưới định kỳ ${result.wateringDays} ngày một lần.`,
    Number(form.temperature) > 55 ? "Cảnh báo: nhiệt độ vượt 55 độ xê, vi sinh có thể chết!" : "",
  ].filter(Boolean).join(" ")
}
...
export default function ResultDisplay({ result, form }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        type="button"
        onClick={() => playing ? stop() : speak(buildResultText(result, form))}
